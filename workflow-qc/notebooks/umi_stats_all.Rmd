...
```{r plot_mt_pct_rank, fig.width=12, fig.height=6, echo=FALSE}
raw_qc_data |> 
  filter(umi_sum > 100) |> 
  ggplot(aes(Sample, mt_pct, fill = cellranger_filtered, colour = Sample)) +
  ...
  scale_fill_manual(
    values = scale_colour_cell_empty
  ) +
  labs(
    title = experiment_title,
    x = "Sample",
...
```{r, summary_after_filtering, echo=FALSE}
summary_after_filtering <- raw_qc_data |> 
  filter(
  ...
  arrange(desc(pass_filters))
kable(summary_after_filtering)
```

```{r, fig.width=12, fig.height=5, echo=FALSE}
ggplot(summary_after_filtering, aes(x = Sample, y = pass_filters)) +
  geom_col(aes(colour = Sample), fill = NA) +
  labs(
    y = "Cells after filtering",
    x = "Sample"
  ) +
  theme_classic() +
  theme(
