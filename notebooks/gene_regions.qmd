...
```{r}
exclude_chr <- gtf_gene_filtered$gene_biotype %in% c("pseudogene", "rRNA", "snoRNA", "snRNA", "transposable_element", "tRNA")
gtf_gene_filtered <- gtf_gene_data[!exclude_chr]
length(gtf_gene_filtered)
```
...
swap <- gene_regions_bed$start > gene_regions_bed$end
gene_regions_bed[swap, c("start", "end")] <- gene_regions_bed[swap, c("end", "start")]
gene_regions_bed
```
